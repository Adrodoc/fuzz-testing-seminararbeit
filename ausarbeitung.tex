...
Eine optimale Testsuite führt demnach jede Anweisung in der getesteten Klasse aus, indem jedes Prädikat mindestens einmal zu \lstinline{true} und \lstinline{false} evaluiert wird.
...
Wenn sich die Fitnessfunktion nur verbessert, wenn mehr Programmpfade abgedeckt werden, ist es sehr unwahrscheinlich, dass eine kleine Änderung die Fitness verbessert, da es sehr schwer ist genau die richtigen Werte zu erraten um ein Prädikat zu erfüllen.
...
Die Idee hierbei ist, dass eine Ausführung eines Prädikats besser ist, wenn die Variablen des Prädikats weniger geändert werden müssen, um es zu erfüllen.
...
Wenn die Variable \texttt{n} den Wert 1 hat, muss sie noch um 16 geändert werden, um das Prädikat zu erfüllen.
...
Tatsächlich ist die Berechnung der \textit{branch distance} etwas komplizierter, so wird z. B. bei Prädikaten, die zu \lstinline{true} evaluieren sollen noch eine Konstante $k > 0$ addiert und es müssen auch komplexe Prädikate mit mehreren Variablen berücksichtigt werden.
...
\begin{figure}[h]
	...
	v(d_{min}(b, T)) & \text{if the predicate has been executed at least twice,}\\
	1 & \text{otherwise.}
	\end{cases}$

	$v(x) = \frac{x}{(x+1)}$
	\caption{Fitnessfunktion \cite{TSE12_EvoSuite}}
\end{figure}
